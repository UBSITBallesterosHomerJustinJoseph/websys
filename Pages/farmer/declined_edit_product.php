<?php
// farmer/declined_edit_product.php
...
// Only allow editing if product is rejected
if ($has_approval_status && isset($product['approval_status']) && $product['approval_status'] !== 'rejected') {
    $_SESSION['edit_error'] = "This page is only for editing rejected products. Please use the regular edit page for pending products.";
    header("location: products.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['resubmit_product'])) {
    // Verify product is still rejected and belongs to farmer
    ...
    } elseif ($has_approval_status && $verify_product['approval_status'] !== 'rejected') {
        $error = "This product can no longer be resubmitted. Only rejected products can be resubmitted.";
    } else {
            ...
            if ($update_stmt) {
                ...
                if ($update_stmt->execute()) {
                    ...
                    $refresh_sql = "SELECT p.*, c.category_name, pi.image_url 
                                    ...
                                    LEFT JOIN categories c ON p.category_id = c.category_id
                                    LEFT JOIN product_images pi ON p.product_id = pi.product_id AND pi.is_primary = TRUE
                                    WHERE p.product_id = ? AND created_by = ?";
                    ...
                    $success = "Product resubmitted successfully! It is now awaiting admin approval again.";
                } else {
                    $error = "Failed to resubmit product: " . $update_stmt->error;
                }
                $update_stmt->close();
            } else {
...
function sendAdminNotification($product_id, $product_name, $farmer_id, $farmer_name) {
    ...
    while ($admin = $admin_result->fetch_assoc()) {
        $notification_sql = "INSERT INTO notifications (user_id, title, message, type, related_id, action_url)
                            VALUES (?, 'Product Resubmitted for Approval', ?, 'approval', ?, ?)";
        $stmt = $farmcart->conn->prepare($notification_sql);

        $message = "Product '{$product_name}' has been resubmitted by farmer {$farmer_name} after being rejected.";
...
<head>
  <meta charset="utf-8">
  <title>Resubmit Rejected Product | FarmCart</title>
    ...
    <div class="main-content-column">
      <div class="content-area">
        ...
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-4 mb-4 border-bottom">
          <div>
            <h1 class="h2 text-warning fw-bold">
              <i class="fas fa-redo me-2"></i>Resubmit Rejected Product
            </h1>
            <p class="text-muted mb-0">Update your product and resubmit it for approval.</p>
          </div>
        ...
        <?php if (!empty($product['admin_notes'])): ?>
          <div class="alert alert-danger mb-4">
            <h6 class="fw-bold mb-2"><i class="fas fa-exclamation-triangle me-2"></i>Previous Rejection Reason:</h6>
            <div class="mb-0"><?= nl2br(htmlspecialchars($product['admin_notes'])) ?></div>
            <small class="text-muted mt-2 d-block">Please address the issues mentioned above before resubmitting.</small>
          </div>
        <?php endif; ?>
            ...
            <div class="row g-4">
              ...
              <div class="col-md-4">
                ...
                <div class="mt-4 p-3 bg-warning bg-opacity-10 rounded border border-warning">
                  ...
                  <p class="mb-0 small">After resubmitting, this product will be reviewed by admin again. Make sure to address the rejection reason above.</p>
                </div>
            ...
            <div class="row mt-4">
              <div class="col-12">
                <div class="d-flex gap-3">
                  <button type="submit" name="resubmit_product" class="btn btn-warning btn-lg px-5">
